...
eval (Variable x) env = 
  case find env x of
    ...
    Param vm -> vm
eval (Apply f es) env =
  eval f env $> (\fv -> apply fv (map (\e -> eval e env) es ))
...
-- INITIAL ENVIRONMENT
values :: [M a] -> M [a]
values ms = foldr consm (result []) ms
    where consm x xs = x $> (\v -> xs $>(\vs -> result (v:vs)))
...
init_env =
  ...
  where
    ...
    primitive x f = (x, Const (Function  (primwrap x g )))
    	where g xs = values xs $> (\vs -> f vs)
