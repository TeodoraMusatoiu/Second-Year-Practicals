...
orelse :: M a -> M a -> M a
orelse xm ym mem kx ks = 
  let ans = xm mem kx ks in case ans of
    ("***exit in main program***", g)  -> ym mem kx ks
    (s, g)                             -> xm mem kx ks
...
elab (Rec x (Lambda xs e1)) env =
  let env' = define env x (abstract xs e1 env' (\ () mem -> ("", (env', mem)) ) )in result env'
